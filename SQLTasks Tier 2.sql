...
SELECT facid, name, membercost, monthlymaitenance
FROM Facilities 
...
SELECT name, monthlymaitenace, 
CASE
    WHEN monthlymaintenace > 100 THEN 'expensive'
    ELSE 'cheap'
END AS cost
FROM Facilities;
...
FROM(
SELECT DISTINCT b.facd, b.memid, f.name
FROM Bookings AS b
...
WHERE
    ...
    AND (
        (b.memid = 0 AND f.guestcost > 30)
        OR (b.memid != 0 AND f.membercost > 30) 
        )
ORDER BY 
    cost DESC;
...
import sqlite3
conn = sqlite.connect('sqlite_db_pythonsqlite.db')
...
c = conn.cursor()
for fow in c.execute(SELECT * FROM Members'):
    print(row)
